...
public class PanoPlayerActivity extends Activity {
    ...
    private static PanoViewWrapper mPanoViewWrapper;
    private ImageView mImgBufferAnim;
    private static PanoPlayerActivity mPanoPlayerActivity;
    private Pano360ConfigBundle configBundle;

    public PanoPlayerActivity (){
        mPanoPlayerActivity = this;
    }

    public static PanoPlayerActivity getmPanoPlayerActivity(){
        return mPanoPlayerActivity;
    }
    @Override
    ...
    private void init(){
        ...
        mPanoUIController.setUiCallback(new PanoUIController.UICallback() {
            ...
            public void pause() {
                if (mPanoViewWrapper.getStatusHelper().getPanoPauseMode()==PanoMode.USER_PAUSE){
                    mPanoViewWrapper.getStatusHelper().setPanoPauseMode(PanoMode.USER_PLAY);
                    mPanoViewWrapper.getStatusHelper().setPanoPauseChangeMode(true);
                } else{
                    mPanoViewWrapper.getStatusHelper().setPanoPauseMode(PanoMode.USER_PAUSE);
                    mPanoViewWrapper.getStatusHelper().setPanoPauseChangeMode(true);
                }

               if (mPanoViewWrapper.getStatusHelper().getPanoStatus()== PanoStatus.PAUSED_BY_USER){
